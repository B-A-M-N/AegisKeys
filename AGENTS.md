...
A local-first, secure terminal app (Go 1.25.13+) that stores API **provider** metadata and **secrets** separately, then injects the correct credentials into coding agents/CLIs. AegisKeys renders **app-specific launch configuration** — env vars, CLI args, and config files — so coding agents get credentials in the exact format each one expects.
...
- **Real-app qualification is separate**: automated adapter verification/fake-executable smoke is not proof that third-party apps accept rendered configuration. Record version-pinned real launches under `testdata/adapter_qualification/` before claiming real-app compatibility.
...
- **Interpreter grants are not app isolation**: executable pins to Python/Node/Ruby/etc. authorize every script run by that interpreter. Refuse by default; require `--allow-interpreter-wide`, persist explicit identity scope, and never represent argv/script paths as a security boundary.
- **Approval transaction service** (`internal/broker/approval.go`): CLI and TUI use the same durable stage/commit/recover path. Session contexts and TUI generation fences cancel stale approvals. Approval intents capture an authoritative policy baseline; recovery reconciles only affected records, preserves administrative/manual policy, and combines it with requirements from currently enabled grants without deriving policy solely from grants.
- **External scratchpad editor is opt-in**: disabled by default. When enabled, it uses Bubble Tea terminal handoff, a shell-free editor argv, a new current-user-owned 0700 runtime directory, and startup cleanup for interrupted AegisKeys editor directories. Preparation messages carry the originating session generation and scratchpad ID; stale plaintext is deleted before any editor launch. Editor backup/swap plaintext remains an explicit limitation.
- **Vault snapshot replacement**: TUI active-vault assignments must use `replaceVaultSnapshot`/`vaultSession.ReplaceVault`; superseded decrypted snapshots are zeroized before replacement, while identical snapshots are left intact.
- **Config cleanup is conflict-aware**: AegisKeys restores only an unchanged managed overlay. A child/user modification after launch is preserved and reported rather than silently overwritten. Filesystem pathname checks are defense in depth, not race-free descriptor containment.
- **Broker lock is a completion barrier**: `Session.Lock()` cancels and invalidates the active generation and does not return until every request admitted before the lock has stopped. Resolve responses and rotate writes are generation-fenced before releasing credentials or success.
- **Provider catalog requests are credential-origin constrained**: authenticated refresh endpoints and every redirect must use HTTPS (except loopback HTTP) and match the provider base origin unless `allow_credential_origin` is explicitly set. Response bytes, model count, and field lengths are bounded.
- **Argon2 resource ceilings are mandatory**: normal decrypt and recovery/rekey paths bound iterations, threads, memory, ciphertext, salt, nonce, and envelope size before derivation. Rekey writes use the shared secure atomic writer, never predictable `.rekey` files.
- **Proxy trust is explicit**: a pre-existing listener is accepted only when AegisKeys can verify its private identity record and live PID. Proxy names are validated; logs and identity files live under private runtime directories with symlink rejection.
- **Unlock is single-flight**: the TUI permits one unlock request, assigns request IDs, invalidates them on lock/quit, and zeroizes every stale successful result.
